...
//성적 출력 함수
void score_output(char*, char**, char**, int**, char***, double*, int, int);
...
int main()
{
	...
	//성적 출력
	score_output("output.txt", name, id, s, g, d, studentNum, subjectNum);
...
void score_sort(double* d, int studentNum)
{
	unsigned int i, j;
...
void score_output(char* fn, char** name, char** id, int** s, char*** g, double* d, int StudentNum, int subjectNum)
{
	FILE* fp;
	unsigned int i, j;
	unsigned int rank = 1;
